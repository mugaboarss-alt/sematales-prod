...
//#region node_modules/.nitro/vite/services/ssr/assets/inquiry-emails.server-D0XGr5d9.js
var RESEND_URL = "https://api.resend.com/emails";
var FROM = "Sema Tales <hello@sematales.rw>";
var OWNER = "hello@sematales.rw";
function escapeHtml(value) {
	return value.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
}
async function sendEmail(payload) {
	const resendKey = processModule.env["RESEND_API_KEY"];
	if (!resendKey) throw new Error("Email credentials are not configured");
...
async function sendInquiryNotification(data) {
	const services = data.services.length ? data.services.map(escapeHtml).join(", ") : "None selected";
	const html = wrap(`
...
async function sendInquiryAcknowledgement(data) {
...
Talk soon,
The Sema Tales Team`;
	const html = wrap(`<p style="margin:0 0 16px;">Hi ${escapeHtml(data.name)},</p>
...
//#endregion
export { sendInquiryAcknowledgement, sendInquiryNotification };
